<template>
  <div>
    <Card dis-hover>
      ...
      <Table size="small" :stripe="true" border ref="tab-bank" :columns="col_bank" :data="data_bank" :loading="this.refreshing"
...
<script>
import { getEmpBankList, delEmpBankByBatch } from '@/api/self-info/info-fill'
...
export default {
  ...
  methods: {
    ...
    deleteByBatch () {
      ...
      delEmpBankByBatch(this.sel_id).then(res => {
        if (res.data.code === '000000') {
          this.sel_id.forEach((id) => {
            for (let [index, row] of this.data_bank.entries()) {
              if (row.id === id) {
                this.data_bank.splice(index, 1)
                break
              }
            }
          })
    ...
    handleDeleteByOne (row, index) {
      ...
      delEmpBankByBatch(condition).then(res => {
        if (res.data.code === '000000') {
          this.data_bank.splice(index, 1)
    ...
    handleRefreshData () {
      ...
      getEmpBankList(condition).then(res => {
        if (res.data.code === '000000') {
    ...
    handleSaveSuccess (isNew, rowIndex, data) {
      if (isNew) {
        this.data_bank.push({
          id: data.id,
          openOrgName: data.openOrgName,
          acctNo: data.acctNo,
          useDesc: data.useDesc
        })
      } else {
        this.data_bank[rowIndex].openOrgName = data.openOrgName
        this.data_bank[rowIndex].acctNo = data.acctNo
        this.data_bank[rowIndex].useDesc = data.useDesc
      }
