<template>
  <div>
    <Card dis-hover>
      <div slot="title">
        <Form ref="formLawsuit" :model="formData" :label-width="70" inline>
      ...
      <Table size="small" :loading="this.load_data && !this.file_uploading" :stripe="true" border ref="tab-lawsuit" :columns="col_lawsuit" :data="data_lawsuit">
        <template slot-scope="{ row, index }" slot="action">
          <Button v-show="row.batNum !== ''" type="error" :to="base_url + 'file/failexport/wadeinfo?batNum=' + row.batNum" target="_blank" size="small"
...
import { col_lawsuit } from './common.js'
...
export default {
  ...
  methods: {
    ...
    handleSearch () {
      ...
      getMainImportList(condition).then(res => {
        ...
        if (data) {
          if (data.code === '000000') {
            this.data_lawsuit = data.data
            this.data_lawsuit.forEach(function (val) {
    ...
    handleBefUpload () {
      ...
      this.load_data = true
      this.data_lawsuit.unshift({ batNum: '', impStatusText: '上传中' })
    },
    ...
    handleUploadSuccess (res, file) {
      if (res.code === '000000') {
        this.data_lawsuit[0].batNum = res.data.batNum
        this.data_lawsuit[0].impDate = res.data.impDate
        this.data_lawsuit[0].impName = res.data.impName
        this.data_lawsuit[0].impStatusText = res.data.impStatusText
  ...
  mounted () {
    ...
    this.template_url = (process.env.NODE_ENV === 'production' ? config.baseUrl.pro_static : config.baseUrl.dev_static) + 'template/涉诉信息导入模板.xlsx'
    this.file_upload_url = this.base_url + config.fileUploadUrl.infoImport + '/wadeinfo'
